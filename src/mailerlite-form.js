document.addEventListener('DOMContentLoaded', function () {
  ...
  function showSuccessBanner() {
    ...
    banner.className =
      'success-banner fixed bottom-0 left-0 right-0 h-[82px] flex items-center justify-center z-50 transform translate-y-full transition-transform duration-300 ease-out';
    banner.style.backgroundColor = '#24775A';

    // Create inner content container
    const content = document.createElement('div');
    content.className =
      'max-w-[1280px] w-full px-6 flex items-center justify-between';
    ...
    closeButton.className =
      'text-white opacity-50 hover:opacity-100 transition-opacity';
  ...
  function showErrorMessage(form, errorText) {
    ...
    let messageContainer = form.parentNode.querySelector(
    ...
    errorMessage.className =
      'form-message text-white text-[1rem] mt-4 px-4 py-3 rounded-lg bg-[#C41E3A]';
    ...
    form.addEventListener('submit', async function (e) {
      ...
      try {
        const response = await fetch(
          '/.netlify/functions/mailerlite-subscribe',
          {
            method: 'POST',
            headers: {
